        ...
        for (let x = 0; x < chunked.length; x++) {
            ...
            pm.addPageEmbed((embed) => {
                embed
                    ...
                    .setColor(this.container.client.config.color)
                    .setFooter({ text: queue.length > 500 ? `Showing up to 500 of ${queue.length} total tracks in queue (Total duration: ${this.humanizeTime(queueDuration)})${looptxt}` : `${queue.length} tracks in queue (Total duration: ${this.humanizeTime(queueDuration)})${looptxt}`});
                return embed;
            });
        }
        pm.run(interaction);
