...
export class RemoveCommand extends Command {
    ...
    async chatInputRun(interaction) {
        ...
        if (index < 1 || index > dispatcher.queue.length - 1) await interaction.reply({ embeds: [this.container.client.util.errorEmbed(`Invalid queue index (accepts **1** to **${dispatcher.queue.length - 1}**).`)] });
